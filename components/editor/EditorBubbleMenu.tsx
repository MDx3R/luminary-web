...
type SelectionSnap = { from: number; to: number };

function getSelectedText(
  editor: Editor,
  snap: React.MutableRefObject<SelectionSnap | null>
): string {
  let { from, to } = editor.state.selection;
  if (
    from === to &&
    snap.current &&
    snap.current.from !== snap.current.to
  ) {
    from = snap.current.from;
    to = snap.current.to;
  }
...
export function EditorBubbleMenu({
  ...
  const selectionSnapRef = useRef<SelectionSnap | null>(null);
  ...
  const handleSendWithComment = useCallback(() => {
    ...
    const selected = getSelectedText(editor, selectionSnapRef);
  ...
  const runAiAction = (fn: (text: string) => void) => {
    const text = getSelectedText(editor, selectionSnapRef);
    fn(text);
    editor.chain().focus().run();
  };
